...
class KunBase:
    """
    Базовый класс для синхронного использования Кунделик API
    """

    def __init__(self, login: str = None, password: str = None, token: str = None):
        self.session = requests.Session()
        self.host = "https://api.kundelik.kz/v2/"
        self.token = token
        if token is None:
    ...
    def get_token(self, login, password):
        ...
        if json_token.get("type") == "authorizationFailed":
            raise KunError(json_token["description"])
    ...
    def _check_response(response):
        if response.headers.get("Content-Type") == "text/html":
            ...
            raise KunError(error_text)
        json_response = response.json()
        if isinstance(json_response, dict):
            if json_response.get("type") == "parameterInvalid":
                raise KunError(json_response["description"])
...
class KunAPI(KunBase):
    ...
    def get_marks_values(self):
        marks_values = self.get(f"https://api.kundelik.kz/v2.0/marks/values")
        return marks_values
    ...
    def get_person_average_marks(self, person_id: int, period_id: int):
        marks = self.get(f"persons/{person_id}/reporting-periods/{period_id}/avg-mark")
        return marks
